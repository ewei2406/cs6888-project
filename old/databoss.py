...
class SQLiteDriver(Driver):
    def run(self, sql: str) -> CommandOutput:
        result = subprocess.run(
            ['sqlite3/sqlite3', 'tmp/db.sqlite', sql], text=True, capture_output=True)
...
class PostgresDriver(Driver):
    def __init__(self) -> None:
        ...
        subprocess.run(["pg_ctl", "-D", "tmp/postgres", "start"],
    ...
    def run(self, sql: str) -> CommandOutput:
        result = subprocess.run(
            ['psql', '-d', 'my_db', '-c', sql], check=True, capture_output=True)
        return CommandOutput.from_subprocess(result)
